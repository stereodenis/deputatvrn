 ...
 */
export const getCurrentCandidate = (person?: Person) => person?.candidate.find((c) => c.callNumber === CURRENT_CALL_NUMBER + 1)
...
export const getPersonWithCurrentDeputat = (areaNumber: number) =>
  persons.find((person) =>
    person.candidate.find((candidate) => candidate.deputat && candidate.callNumber === CURRENT_CALL_NUMBER && candidate.areaNumber === areaNumber)
  )
...
export const getAreaCandidates = (areaNumber: number) =>
  currentPersons.filter((p) => p.candidate.find((c) => c.areaNumber === areaNumber))
